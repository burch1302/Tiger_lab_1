...
int main(int argc, char **argv) {
  ...
  ("eval", "evaluate the AST and print the result")
  ...
  bool eval_flag = vm.count("eval") > 0;
  bool dump_ast_flag = vm.count("dump-ast") > 0;
  ...
  if (vm.count("dump-ast")) {
    ...
    if (eval_flag && dump_ast_flag) {
    utils::error("Cannot use both --eval and --dump-ast");
  }

  if (eval_flag) {
    ast::Evaluator evaluator;
    int value = parser_driver.result_ast->accept(evaluator);
